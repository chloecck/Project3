    ...
    try:
        ...
        with lock:
            ...
            if (user_id and not user_key) or (not user_id and user_key):
                return {
                    "err": "if provide a user, it must contain both user_id and user_key"
                }, 400
            if user_id and user_key:
                user_profile = users.get(user_id, None)
                if not user_profile:
                    return {
                        "err": "no user found with the user_id in the post POST method"
                    }, 404
    ...
    with lock:
        ...
        if key != post["key"]:
            if "user_key" in post and key != post["user_key"]:
                return {
                    "err": "provided key does not match post's key nor user's key"
                }, 403
...
def create_user():
    ...
    if (
        ...
        or ("real_name" not in data)
        or (not isinstance(data["real_name"], str))
    ):
        return {
            "err": "should be json object w username and real_name string field"
        }, 200
...
def edit_user_metadata(user_id, user_key):
    try:
        ...
        if (
            ...
            return {
                "err": "should be json object with both username and real_name string field"
            }, 400
        with lock:
            ...
            user_profile["real_name"] = data.get("real_name", user_profile["real_name"])
